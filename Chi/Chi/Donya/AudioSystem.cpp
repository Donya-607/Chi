...
#include "Constant.h"	// Use DEBUG_MODE marco.
#include "Useful.h"		// Use OutputDebugStr(), and convert character-code functions.
	...
	private:
		...
		bool AppendFadePoint( float takeSecond, float destVolume, bool applyAll )
		{
			...
			#pragma endregion
				...
				double dMixerRate  = static_cast<double>( mixerRate  );
				double dTakeSecond = static_cast<double>( takeSecond );
				distance = static_cast<unsigned long long>( dMixerRate * dTakeSecond );
